...
def soft_nms(bboxes, scores, iou_thresh=0.5, sigma=0.5, score_threshold=0.25):
    ...
    while order.numel() > 1:
        ...
        iou = box_iou_for_nms(bboxes[i], bboxes[order[1:]],SIoU=True).squeeze()  #选择Siou
...
def NMS(boxes, scores, iou_thres, GIoU=False, DIoU=False, CIoU=False, SIoU=False):
    """
    ...
    while B.numel() > 0:
        ...
        keep.append(index)
        if B.numel() == 1: break
        # 计算iou,根据需求可选择GIOU,DIOU,CIOU,SIoU
        iou = bbox_iou(boxes[index, :], boxes[B[1:], :],  SIoU=True)
        # 找到符合阈值的下标
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS训练
        #i = soft_nms(boxes,scores,iou_thres)#softnms 检测
        #i = NMS(boxes, scores, iou_thres,SIoU=True)
